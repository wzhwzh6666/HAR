...
def loss_sort(y_1, t, forget_rate, criterion):
    loss_1 = criterion(y_1, t)  
    loss_1 = torch.mean(loss_1, dim=1)
    #print("loss_1:", loss_1)
    
    # 排序损失值
    ind_1_sorted = np.argsort(loss_1.cpu().data.numpy())  # 根据损失值升序排列
    loss_1_sorted = loss_1[ind_1_sorted]
    #print("ind_1_sorted:", ind_1_sorted)
    #print("loss_1_sorted:", loss_1_sorted)

    remember_rate = 1 - forget_rate  # 保留样本的比例
    num_remember = int(remember_rate * len(loss_1_sorted))
    #print("num_remember:", num_remember)

    ind_1_update = ind_1_sorted[:num_remember]  # 选择要保留的样本的索引
    #print("ind_1_update:", ind_1_update)
    
    # 计算选择样本的损失
    loss_1_update = criterion(y_1[ind_1_update], t[ind_1_update])
    #print("loss_1_update:", loss_1_update)
    
    # 返回更新后的损失值均值
    return torch.sum(loss_1_update) / num_remember
